...
{
    public class AdoNetQualitiesRepository : IQualitiesRepository
    {
        private readonly SteamMarketplaceDbContext _context;

        public AdoNetQualitiesRepository(SteamMarketplaceDbContext context)
        {
            _context = context;
        }
        ...
        public Guid GetQualityId(string name)
        {
            if (string.IsNullOrEmpty(name))
            {
                throw new ArgumentNullException("name", "The name must not be empty.");
        ...
        public bool Save(Quality entity, bool checkOnUnique = true)
        {
            if (entity == null)
            {
            ...
            if (!checkOnUnique || !Contains(entity.Name, out foundId))
            {
                ...
                var query = $"INSERT INTO \"Qualities\" (\"Id\", \"Name\", \"RuName\") VALUES (@Id, @Name, @RuName)";
            ...
            {
                entity.Id = Guid.Empty == foundId ? GetQualityId(entity.Name) : foundId;
            }
